...
dbProviders <- bdInternet %>%
  ...
  summarise(promDown = mean(DOWNLOAD_INDICATOR),
...
bdInternet %>%
  group_by(PROVIDER) %>%
  summarise(promDown = mean(DOWNLOAD_INDICATOR),
            desvDown = sd(DOWNLOAD_INDICATOR),
            promUp = mean(UPLOAD_INDICATOR),
            desvUp = sd(UPLOAD_INDICATOR))
```
```{r}
plot_correlate(select(bdInternet, LATENCY_MS, DISTANCE_MILES))
plot_correlate(select(bdInternet, DOWNLOAD_INDICATOR, DISTANCE_MILES))
plot_correlate(select(bdInternet, UPLOAD_INDICATOR, DISTANCE_MILES))
```
